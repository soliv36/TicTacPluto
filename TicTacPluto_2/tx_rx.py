...
def tx_samples(sdr, samples):
    logging.info('Transmitting samples')
    sdr.tx_destroy_buffer() #clear the txbuffer
...
def rx_text(sdr, Game):
    logging.info('Receiving Samples')
    ...
    while True:
        ...
        if start_index > 0:
            ...
            if end_index != -1:
                ...
                if (tx_rx.is_valid_text(text)) and (text != []) and (text != Game.current_move): #prevent receiving a move already played/make stopping tx more accurate
